...
class Serenity_Lite_Welcome {
	...
	public function __construct() {
		...
		add_action( 'serenity_lite_welcome', array( $this, 'serenity_lite_welcome_import_demo' ), 		    30 );
		add_action( 'serenity_lite_welcome', array( $this, 'serenity_lite_welcome_services' ), 		    	40 );
	...
	public function serenity_lite_activation_admin_notice() {
		...
		if ( is_admin() ) {
			...
			wp_enqueue_style( 'serenity-lite-welcome-notice-css', get_template_directory_uri() . '/inc/welcome/css/notice.css', array(), '1.2.5' );
	...
	public function serenity_lite_welcome_admin_notice() {
		...
		$dismiss_url = '<a href="' . esc_url( wp_nonce_url( add_query_arg( 'serenity_lite_welcome_dismiss', '1' ) ) ) . '" target="_parent">Dismiss this notice</a>';
		?>
			...
			<div class="notice theme-notice">
				...
				<div class="theme-notice-message wp-theme-fresh">
					<strong><?php esc_html_e( 'Thank you for choosing Serenity Lite!', 'serenity-lite' ); ?></strong><br>
					<?php esc_html_e( 'Read our instructions to setup your new theme and begin customizing your site.', 'serenity-lite' ); ?><br>
					<?php echo $dismiss_url ?>
				</div>
				<div class="theme-notice-cta">
					<a href="<?php echo esc_url( admin_url( 'themes.php?page=serenity-lite-welcome' ) ); ?>" class="button button-hero" style="text-decoration: none;"><?php esc_html_e( 'Setup Instructions', 'serenity-lite' ); ?></a>
					<a href="<?php echo esc_url('https://support.themely.com/services/'); ?>" target="_blank" class="button button-hero" style="text-decoration: none;"><?php esc_html_e( 'Hire Us', 'serenity-lite' ); ?></a>
					<a href="<?php echo esc_url('https://www.themely.com/themes/serenity/'); ?>" target="_blank" class="button button-hero" style="text-decoration: none;"><?php esc_html_e( 'Upgrade to Serenity Pro', 'serenity-lite' ); ?></a>
				</div>
			</div>
			<?php endif; ?>
		<?php
	...
	public function serenity_lite_welcome_style_and_scripts( $hook_suffix ) {
		...
		if ( 'appearance_page_serenity-lite-welcome' == $hook_suffix ) {
			wp_enqueue_style( 'serenity-lite-welcome-screen-css', get_template_directory_uri() . '/inc/welcome/css/welcome.css', array(), '1.2.5' );
			wp_enqueue_script( 'serenity-lite-welcome-screen-js', get_template_directory_uri() . '/inc/welcome/js/welcome.js', array('jquery'), '1.2.5' );
		}
	}
	...
	public function serenity_lite_welcome_screen() {
		...
		<div class="wrap about-wrap theme-welcome">
            ...
            <h1><?php esc_html_e('Welcome to Serenity Lite', 'serenity-lite'); ?> <span><?php esc_html_e('VERSION 1.2.5', 'serenity-lite'); ?></span></h1>
			...
			<ul class="nav-tab-wrapper" role="tablist">
	            ...
	            <li role="presentation" class="nav-tab"><a href="#customization_services" aria-controls="customization_services" role="tab" data-toggle="tab"><?php esc_html_e( 'Hire Us','serenity-lite'); ?></a></li>
	        </ul>
	 ...
	 * Import Demo
	 */
	public function serenity_lite_welcome_services() {
		require_once( get_template_directory() . '/inc/welcome/customization-services.php' );
	}
